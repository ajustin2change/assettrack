...
2. `curl` the live bytes; byte count + md5 must match the line above. Mismatch = STOP and say so.
3. NO baseline gate run — matching md5 means the delivered bytes the gates already passed. Gates run on EDITED bytes only, plus compile-verify, on every build. Function-boundary map only before scoping a UI diff.
4. Feature builds: plan/mockup approval before code. Bugfixes: present the diagnosis, then ship — the diagnosis is the design.
5. Deliver: surgical Python apply script with exact-match assertions from the current deployed bytes + built `index.html` + any extended gate files.
6. After Justin confirms live smoke: REPLACE this STATE block, APPEND a session block to the archive. Never before.
...
> **ARCHIVE.** Superseded as the fast path by the ⚡ STATE block above — read a session block here only when a build touches its territory. Detailed findings are updated in place (✅ = done). When archive text conflicts with STATE, STATE wins.
...
**Discrepancy-tracker session — COMPLETE (Builds D0–D3.1 all shipped + live-smoked):**
...
- ✅ **Build D2.1 (first-use feedback + a posture fix): SHIPPED + LIVE-SMOKED.** (1) Collapsed-row state pill is DISPLAY-ONLY; state changes are explicit labeled buttons in the expanded panel ("Mark as Chasing" / "Back to Open") — the silent-toggle pill failed discoverability AND feedback. (2) **RULE: automation may only reopen what automation closed.** The harvest's reopen branch is guarded on `resolution.resolvedBy==="system"` — a human resolution (vendor email, judgment call) is never contradicted by a report that merely STILL lists the serial; old evidence is not new evidence. System-resolved records still reopen on reappearance — state follows evidence in both directions, but only inside the evidence domain. (3) **Seed REMOVED entirely** (button, empty-state block, App handler): its only job was the pre-tracker transition (the 5 already-reconciled pickups), completed on its first click; every future record — including all backfill — enters through live reconciles. Dead scaffolding stripped per the lean rule. Disaster-recovery note: if `discrepancies` were ever lost, re-reconciling pickups regenerates OPEN records, but notes/states are not re-derivable from reports. Guard live-smoked: a human-resolved item survived a re-reconcile of the same audit file untouched while other records' `lastSeenAt` advanced (harvest ran and deliberately skipped it). **Deployed at D2.1: 400,644 bytes, md5 54638e02f1709433b108f138f632cd81.**
- ✅ **Build D3 (Track button): SHIPPED.** Fifth column on the report's Extra-from-Vendor table (which lives in `ReconcilePickup`, NOT `ReportTab` — see D3.1). Clicking Track promotes a vendor extra into the tracker: App-level `trackExtra` locates the owning pickup STRICTLY by `reconcileReport.generatedAt` match, snapshots vendor tag/desc/mfgr as evidence, and attributes the opening note to the operator's email — tracking an extra is a human judgment, so it gets human attribution. Already-tracked rows render "Tracked ✓" (no double-promotion). The harvest already counts still-listed extras as present, so a tracked extra survives re-reconciles without duplication or premature auto-resolve.
- ✅ **Build D3.1 (hotfix): SHIPPED + LIVE-SMOKED.** D3 wired the new props into `ReportTab`, but the extras table actually lives in `ReconcilePickup` → runtime `ReferenceError: discrepancies is not defined` when opening a pickup on the Reconcile tab. Fix: reverted the dead `ReportTab` wiring and threaded `discrepancies`/`onTrackExtra` through the real chain App → `ReconcileTab` → `ReconcilePickup`. **LESSON, twofold:** (1) component ownership must be verified by function-boundary map (`awk '/^function [A-Z]/'`), never inferred from grep proximity — now a required step before scoping any UI diff; (2) this bug class is INVISIBLE to compile-verify and all three gates (valid syntax, fails only at render) — see the promoted mount-smoke item in §8. **Deployed: 403,036 bytes, md5 bc47469468ad7c9a6b93f52036a54322.** All three gates green on every build; compile-verified each time.

**Completed in the pickup-resolution session (BUILD 1a — deployed and smoke-tested live; preceded by two design sessions that produced the full Accept/Rescan/Reject + two-exit model):**

- **Two live incidents drove this work.** (1) Ticket 660123 (7/17): rejected fuzzy corrections still rendered as Proposed Corrections on both approval PDFs, the WRONG (mistyped) scanned serials were saved to assets as pending extras while the customer's CORRECT serials vanished from the record, and the re-download PDF double-listed each rejected serial as both a Correction and an Extra. (2) A 7/21 pickup: an UNTOUCHED (pending) fuzzy correction fell through the same silent else-branch in `buildFinalAssets` — double-listing with zero rejects. Root cause for both: the branch was binary (`accept` vs everything-else) and no surface honored `resolution:"reject"`.
- **Design model settled with Justin (governs Builds 1b–3):** *"the ticket records what I picked up; the signed document accounts for what the customer submitted."* **Accept** = correction confirmed, record the corrected serial. **Reject** = the device was NOT taken — capture deleted outright (never persists to `pickupScanList`), expected serial becomes **Missing** for customer acknowledgment. **Rescan** (Build 2) = "I have the device, my capture was wrong" — removes the bad capture, focused manual re-entry ONLY (NO use-ticket-serial shortcut — rejected as a falsification vector), compare re-runs in place. **Two exits from Compare (STRICT model, Build 1b):** Exit A customer-present — every discrepancy decided + signature → straight to verified, a SIGNED approval PDF filed, custody event; Exit B send-for-approval — in-person clicks are CLEARED and every discrepancy is restated fresh (the signed document is always the complete decision record, never a partial one). Undecided is a state that can exist on screen but can never exit.
- ✅ **`finalizePickup` core (top-level, extractable).** The ONE place decision consequences are applied — replaces the component-local `buildFinalAssets` (deleted, sole caller rewritten). Accept → asset under canonical with expected metadata; reject → capture into `rejectedCaptures` (pruned from the saved scan list) + expected serial appended to missing with `fromRejected:true`; pending → stays a proposed correction in `pickupResult`, NEVER enters assets. Assets can now contain ONLY: matched, accepted corrections, linked extras, true unlinked extras (pending).
- ✅ **Reject honored on every surface:** `actionableCorrections` now excludes `resolution==="reject"` rows — scan view, awaiting-approval panel, approval email counts, approval XLSX, and BOTH approval PDFs, in one filter.
- ✅ **Save guard:** Save & Send is disabled with an amber "N correction(s) undecided" count until every partial is accepted or rejected. (Interim rule until Build 1b's exits; Exit B will later relax this by restating instead.)
- ✅ **Re-download PDF extras now derive from `pickupResult` buckets** — the same source as the send-time PDF — instead of `assets` filtered on `scan-extra-pending`. Double-listing is structurally impossible.
- ✅ **Housekeeping shipped:** dead `runFolderImport` and `downloadSessionJson` deleted (the latter was the D3-class `locations`/`requestors` ReferenceError carrier).
- ✅ **`pickup_tests.js` added to the repo — FOURTH self-extracting gate.** 16 cases pin the resolution core: both incident regressions by name (reject leaks nothing to assets under either serial; pending never becomes a phantom extra; no serial may appear as both a correction and a pending extra), accept/canonical/metadata rules, linked/unlinked extras, missing preservation, the reject surface filter, and the no-op boundary. **REQUIRED gate: `node pickup_tests.js index.html` after ANY edit to `finalizePickup`, `actionableCorrections`, `isNoopCorrection`, `saveAndClose`, or any surface rendering corrections/missing/extras.**
- ✅ **`mount_smoke.js` (Gate 4) REBUILT and finally IN THE REPO** (it previously existed only on Justin's machine — the repo 404'd). Babel-parses the `jsx-source` block and scope-walks for undefined names; harvests page globals (`auth`, `repo`, …) from the plain bootstrap `<script>` blocks automatically. Negative-controlled against the prior deployed build: correctly catches `locations`/`requestors` at the dead function — the exact D3-incident bug class it was promoted for. Needs `npm i @babel/core @babel/preset-env @babel/preset-react @babel/parser @babel/traverse` (dev machine only). **REQUIRED gate on every build.**
- **Delivery artifacts:** `apply_build1a.py` (8 hunks, exact-match assertions, reproducible from the 403,036-byte base). **Deployed: 401,034 bytes, md5 8bb79e0ac32b7377959686eb72b396a8.** All FIVE gates green (matcher 17, detector 9, reconcile 46, pickup 16, mount clean); compile-verified.
- **Also agreed for upcoming builds (copy rules):** all serial inputs read "Serial numbers" (no Excel/paste mentions); UI direction text stays succinct — no redundant explanations.

**Completed in the two-exits session (BUILDS 1b + 1c — both deployed and smoke-tested live; 1c fixed the three bugs 1b's smoke surfaced):**

- ✅ **The Compare page is now the decision surface for ALL THREE discrepancy types (Build 1b).** Corrections keep Accept/Reject; Missing rows gain **✓ Kept** ("customer kept the item — confirmed not picked up"; the open design question resolved as ONE verb, not two); Extra rows gain **Include / Remove**. Every clicked decision shows an Undo chip until an exit is taken. Decisions live in the same `resolutions` map under the deterministic row ids; `getResolved()` reads them for missing/extras (links still win).
- ✅ **Exit A — customer present (STRICT gate, terminal).** "Customer Approval — In Person" card above the send button: `SignaturePad` (canvas, pointer events; light "paper" background + dark ink so the exported PNG stays legible on the white PDF), printed name, date. **Sign & Verify is disabled until every discrepancy is decided AND ink AND name** — `undecidedCounts()` (top-level, extractable) is the gate. On sign: consequences apply through `finalizePickup`, ticket goes STRAIGHT to verified (`approvalStatus:"validated"` + `validatedAt` + `approvalSignature{dataUrl,printedName,signedAt}` stored ON the ticket — the PDF itself is never stored, CoDD rule), the SIGNED PDF downloads automatically (header "Approval Record (Signed)", Decision column pre-marked — removed extras DENIED, everything else APPROVED; rejected corrections surface as Missing APPROVED rows via `finalizePickup`, which is the correct semantic: rejecting the correction asserts the on-ticket device wasn't taken), rejected captures pruned from the scan list, `expectedAssets` synced + `originalAssets` frozen (mirroring validation).
- ✅ **Exit B — send for approval (strict restatement).** The 1a undecided save-guard is retired; Save & Send is always enabled. `restateForApproval()` (top-level) wipes **clicked decisions only** — corrections, missing, extras all restate fresh on the sent PDF; machine pre-acceptance (exact-tier `defaultResolution`) and link pairs SURVIVE the wipe (they are the matcher's and operator's determinations, not the customer's), so no-op corrections stay approval-exempt and are never restated. Pendings remain proposed corrections and never enter assets.
- ✅ **The awaiting-approval panel speaks the same verbs.** The scattered approve/deny/undo handlers (asset-level flags) are GONE; one `setPrRes(bucket,id,verb)` writes decisions onto `pickupResult` items. Corrections get Accept/Reject/Undo (they now matter — Exit B leaves them pending); missing get **Confirm Kept**; extras get Include/Remove and are sourced from `pickupResult.extra`, NOT the assets list. **Mark Validated is gated** on `undecidedCounts` (undecided can never exit — alert names the count) and applies ALL consequences through `finalizePickup`, including scan-list pruning of rejected captures. **TRANSITION NOTE: tickets sent under 1a must have extras/missing re-decided with the panel verbs — the old asset-level `approved` flags are no longer read at validation.**
- ✅ **`finalizePickup` extended (same one-place rule):** missing `resolution:"kept"` → `approvedMissing:true`; extra `include` → `scan-extra` approved asset; `remove` → no asset (capture stays in `pickupScanList` only); undecided extra → pending asset unchanged.
- ✅ **Custody: `approval-captured` (Exit A) + `approval-validated` (1c, remote).** Both wrapper-detected in `detectTicketCustody` — pure Option A, no new emission sites. `approval-captured` rides the `approvalSignature` APPEARING (attester = the signer, email name-matched from requestors; signature image NOT in the event). `approval-validated` rides `approvalStatus → "validated"` when there is NO signature AND NOT `directVendorPickup` (so Exit A stays approval-captured and attested direct pickups stay pickup-attested — one event per approval, exclusions pinned in the gate). Attester = requestor via `attesterFor`; note carries the uploaded approval filename when present. Timeline meta added for both. The detector comment now names `approvalSignature`/`approvalStatus` as semantic dependencies.
- ✅ **Rescan restores the ORIGINAL submission (1c).** The 1b smoke's key find: validation syncs `expectedAssets` to the approved outcome, so rescanning a validated ticket compared against the approved list and reproduced none of the original errors. `doRescan` now restores `expectedAssets` from `originalAssets` (frozen at first validation — the original was never lost) and clears `approvalSignature`/`validatedAt`, so rescan → re-approve honestly emits a second custody event. Works retroactively for tickets validated before the fix.
- ✅ **Re-download prints a RECORD once validated (1c).** `regenApprovalPdf`: any validated ticket — signed or not — prints "IT Asset Disposal — Approval Record" with the Decision column filled from recorded decisions (include→APPROVED, remove→DENIED, kept→APPROVED, accepted corrections→APPROVED); "(Signed)" suffix + embedded signature image only when `approvalSignature` exists. Awaiting tickets still print the blank-decision Request — the strict model's restatement rule.
- ✅ **Copy rules applied:** both serial-entry placeholders read "Serial numbers".
- ✅ **Gates extended (pinned before shipped):** `pickup_tests.js` **16 → 27** (extra verbs incl. removed-never-rejectedCaptures, kept-missing, `undecidedCounts` boundaries incl. legacy `approvedMissing`, the Exit B wipe incl. defaultResolution/link survival, restate-then-finalize incident regression, signed-record no-pending-sources invariant). `detector_smoke.js` **9 → 12 (1b) → 16 (1c)** (approval-captured emit-once/silent-resave/no-spurious-on-scan-flip; approval-validated with attester+filename note; Exit A emits approval-captured ONLY; direct-pickup exclusion; validated-resave silence). All five gates green on every build; compile-verified each time.
- **1b smoke also surfaced (diagnosed, deliberate no-fix):** the Mark Validated ticket WRITE was persisting all along — the missing custody event was the visible symptom (confirmed by reload check). `generateApprovalSheet` (the approval XLSX) confirmed DEAD CODE during the 1b reads — housekeeping-deletion candidate, untouched.
- **Delivery artifacts:** `apply_build1b.py` (45 hunks, from the 401,034-byte base → 414,294 bytes / md5 1d50ae5f5dd94caee7d8455104bec89b), `apply_build1c.py` (10 hunks, from the 1b bytes). **Deployed: 415,463 bytes, md5 1c4caa6868846914c373b942bb3f3feb.** Updated `pickup_tests.js` + `detector_smoke.js` committed alongside.
...
- **Bug list: CLOSED again (two-exits session).** The 1b live smoke surfaced three bugs (rescan compared against the approved list; no custody event on remote validation; re-download printed decisions blank on validated tickets) — all fixed in 1c and gate-pinned. No known open bugs at handoff time.
- **NEXT BUILDS (designed, in order):** ~~Build 1b~~ SHIPPED (+1c bugfixes — see the two-exits session block). **Build 2** — Rescan on correction AND extra rows (banner "Rescanning — ticket expects X", manual scan/type ONLY, in-place recompute preserves sibling decisions). **Build 3** — live matcher helper (same primitives as `bestMatch`, under the `matcher_tests.js` gate) powering BOTH the scan-page truth fix (counter "N matched · M likely · K unaccounted"; amber "likely match" row status replacing the misleading Pending on captured-but-fuzzy serials — the 21-laptop confusion incident) and the no-match "thunk" sound cue (WebAudio, fires ONLY on true no-match — nothing exact/token/containment/fuzzy; fuzzy-matchable scans stay SILENT by explicit decision; mute toggle; amber chip as visual residue). **Then: ticket 660123 data repair** (Rescan Ticket → re-enter scan list → decide correctly → re-send).
...
- **[SHOULD] (durable)** **Thin test net:** the scan matcher (`bestMatch`/`extractIdentifiers`) and the no-op rule HAVE standalone tests in `matcher_tests.js`, and the custody emission detectors HAVE `detector_smoke.js` (both self-extracting from `index.html` — NOT copies, cannot go stale), and the reconcile core is now PINNED by `reconcile_tests.js` (46 cases against real-audit fixture shapes — reconcile-tests session; `flexMatch`, `findCol`, `isBlankTag`, `normTkt`, and the full `runReconciliation` closure all covered). ✅ **MOUNT SMOKE SHIPPED (pickup-resolution session):** `mount_smoke.js` is in the repo — AST scope-walk of the `jsx-source` block for undefined names (the D3 ReferenceError class), page globals auto-harvested, negative-controlled against the D3-class bug. The pickup resolution core is pinned by `pickup_tests.js` (fourth logic gate). REMAINING in this item: ideally extract the shared matcher so app + tests use one source.
