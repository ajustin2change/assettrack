...
- **Bug list: CLOSED again (pickup-resolution session).** The two resolution-flow incidents (660123 reject leak; 7/21 pending double-listing) are fixed and regression-pinned in `pickup_tests.js`. No known open bugs at handoff time.
- **NEXT BUILDS (designed and mocked up, in order):** **Build 1b** — two exits from Compare: Exit A in-person (all-decided gate + signature pad + name/date → verified, SIGNED approval PDF pre-marked APPROVED/DENIED filed with the ticket, custody event for in-person approval capture) and Exit B strict restatement (clears in-person clicks, restates every discrepancy); the awaiting-approval view gets the same decision verbs via `finalizePickup`'s model for all three discrepancy types. **Build 2** — Rescan on correction AND extra rows (banner "Rescanning — ticket expects X", manual scan/type ONLY, in-place recompute preserves sibling decisions). **Build 3** — live matcher helper (same primitives as `bestMatch`, under the `matcher_tests.js` gate) powering BOTH the scan-page truth fix (counter "N matched · M likely · K unaccounted"; amber "likely match" row status replacing the misleading Pending on captured-but-fuzzy serials — the 21-laptop confusion incident) and the no-match "thunk" sound cue (WebAudio, fires ONLY on true no-match — nothing exact/token/containment/fuzzy; fuzzy-matchable scans stay SILENT by explicit decision; mute toggle; amber chip as visual residue). **Then: ticket 660123 data repair** (Rescan Ticket → re-enter scan list → decide correctly → re-send).
